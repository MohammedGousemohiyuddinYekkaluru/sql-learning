...
New Column 2: Overall_Rank — Rank every single sale in the company based on quantity_sold (highest quantity = Rank 1).
*/
...
SELECT
	...
	p.product_name,
	s.quantity_sold,
	RANK() OVER (PARTITION BY s.store_location ORDER BY s.quantity_sold DESC) Store_Rank,
	RANK() OVER (ORDER BY s.quantity_sold DESC) Overall_Rank
FROM Sales as s
